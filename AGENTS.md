...
Typography operating rule: the formal Font Size source of truth is `assets/css/user-settings.css`, and the normal user-editing surface is only `USER EDITABLE — TYPOGRAPHY`. Same-tag normal content uses a shared role within each page group: Home (`index.html`), Standard pages (Gallery, Biography, Artist Statement, Information, Order, Contact, Policy and Yurayura), and 404. Current group tokens include Home `--type-home-h1-size` / `--type-home-p-size`, Standard `--type-page-h1-size` / `--type-page-p-size` / `--type-page-li-size`, and 404 `--type-404-title-size` / `--type-404-p-size`. Page names, classes and IDs alone do not justify separate same-tag normal-content sizes. Normal font sizes follow the existing 375px-to-1440px fluid policy; do not add viewport-specific sizes or new breakpoints solely for typography.

Permitted font-size exceptions are limited to clearly distinct component roles such as header/navigation, menu, footer, interactive links, form controls, buttons, modal controls, Gallery cards, table/metadata text, captions/helpers, animated Home display text and 404 code; document the reason. Letter-spacing, tracking, 404 optical treatment, component helpers and legacy/non-Home optical tokens are separate concerns from group typography and must not be bulk-deleted. Current group tokens use the generic starting values specifically selected by the user. Final Font Size tuning belongs to the user; AI/Codex must not numerically optimize or redesign those values without an explicit request. Compatibility aliases are not independent edit controls.
