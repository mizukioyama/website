document.addEventListener("DOMContentLoaded", function () {
  ...
  style.textContent = `
    ...
    .request-options legend { margin-bottom: .85rem; font-size: clamp(12.8px, calc(11.2px + .35vw), 16px); font-weight: 300; letter-spacing: 1px; opacity: .65; }
    ...
    .request-option-list label { display: flex; flex-direction: column; justify-content: center; width: max-content; max-width: 100%; min-width: 0; min-height: 0; margin: 0 !important; padding: 4px 0; border: 0; border-bottom: 1px solid transparent; background: transparent; box-shadow: none; backdrop-filter: none; -webkit-backdrop-filter: none; font-size: clamp(12px, calc(10.88px + .3vw), 15.2px); line-height: 1.45; letter-spacing: 1px; font-weight: 300; opacity: .55; cursor: pointer; transition: opacity .25s ease, border-color .25s ease; box-sizing: border-box; }
    .request-option-list label span { font-size: clamp(10.56px, calc(9.5744px + 0.264vw), 13.376px); }
    ...
    .form-status { min-height: 1.5em; margin: 1rem 0 0; font-size: clamp(11.2px, calc(9.92px + .3vw), 13.6px); opacity: .7; }
